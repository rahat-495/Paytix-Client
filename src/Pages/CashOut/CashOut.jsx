...
const CashOut = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const amount = parseFloat(form.amount.value) * 0.015 + parseFloat(form.amount.value) ;
    const sendingData = {
      sendingInfo,
      amount,
      ...data,
    ...
    if (amount >= 50) {
      Swal.fire({
        title: "Are you sure?",
        html: `You won't be able to send <br/> Cash Out request `,
        icon: "warning",
        ...
        if (result.isConfirmed) {
          axiosSecure.post("/cashOut", sendingData).then((res) => {
            if (res?.data?.insertedId) {
              Swal.fire({
                title: "Cash Out",
                text: "Cash Out Request Sended .",
                icon: "success",
              });
              form.reset() ;
            } else {
  ...
  return (
    <div className="min-h-[80vh] flex flex-col w-full mx-auto items-center justify-center">
      <h1 className="play text-4xl mb-10 font-semibold">Cash Out</h1>
